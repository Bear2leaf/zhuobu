...
var ___emscripten_embedded_file_data = Module["___emscripten_embedded_file_data"] = 1930644;

var ___start_em_js = Module["___start_em_js"] = 2051056;

var ___stop_em_js = Module["___stop_em_js"] = 2056060;
...
// comment out the following line to enable the shell environment
// assert(!ENVIRONMENT_IS_SHELL, "shell environment detected but not enabled at build time.  Add 'shell' to `-sENVIRONMENT` to enable.");
// after the end of the nethack.js, add the following lines:
...
let nethackCallbackResolver = null;

typeof worker === 'undefined' ? onmessage = function (e) { nethackCallbackResolver(e.data) }: worker.onMessage((data) => nethackCallbackResolver(data));
globalThis.nethackCallback = async (...data) => { 
  typeof worker === 'undefined' ? postMessage(data) : worker.postMessage(data);
  return await new Promise((resolve) => nethackCallbackResolver = resolve)

} 
if (typeof worker !== 'undefined') {
    const wxinst = WXWebAssembly.instantiate;
    globalThis.WebAssembly = WXWebAssembly
    globalThis.WebAssembly.instantiate = (path, info) => wxinst(String.fromCharCode(...path), info);
...
Module({
  arguments: [],
  locateFile: () => `../resources/${filename}.wasm`,
  wasmBinary: typeof worker === 'undefined' ? undefined : [...`resources/${filename}.wasm`].map(c => c.charCodeAt(0)),
