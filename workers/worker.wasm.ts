...
if (typeof worker !== 'undefined') {
  ...
  globalThis.WebAssembly.instantiate = (path: any, info) => wxinst(String.fromCharCode(...path), info);
  globalThis.WebAssembly.RuntimeError = Error

}
const filename = 'static/wasm/ammo.wasm';
var config = {
  locateFile: () =>  `../${filename}.wasm`,
  wasmBinary: typeof worker === 'undefined' ? undefined : [...`${filename}.wasm`].map(c => c.charCodeAt(0))
}
Ammo.bind({})(config).then(function (Ammo) {
  ...
  function simulate(dt: number) {
    ...
    var data : {objects: number[][], currFPS: number, allFPS: number} = { objects: [], currFPS: Math.round(1000 / meanDt), allFPS: Math.round(1000 / meanDt2) };
  ...
  if (typeof worker !== 'undefined') {
    ...
    worker.postMessage({ data: { isReady: true } });
  } else {
