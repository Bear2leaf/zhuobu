...
import { EventsEncounters } from "./events/encounters.js";
import { EventsExecutioner, _LEAVE_COOLDOWN } from "./events/executioner.js";
import { EventsGlobal } from "./events/global.js";
import { EventsMarketing } from "./events/marketing.js";
import { EventsOutside } from "./events/outside.js";
import { EventsRoom } from "./events/room.js";
import { EventsSetpieces } from "./events/setpieces.js";
...
export default class Events {
	...
	static loadScene(name: string) {
		...
		adr.$('#description', Events.eventPanel()).empty();
		adr.$('#buttons', Events.eventPanel()).empty();
		if (scene.combat) {
	...
	static startCombat(scene: ADREvent) {
		...
		Events.fought = false;
		Events.won = false;
		const desc = adr.$('#description', Events.eventPanel());

		adr.$('<div>').text(scene.notification).appendTo(desc);


		const fightBox = adr.$('<div>').attr('id', 'fight').appendTo(desc);
		// Draw the wanderer
		...
		const btns = adr.$('#buttons', Events.eventPanel());

		const attackBtns = adr.$('<div>').appendTo(btns).attr('id', 'attackButtons');
		let numWeapons = 0;
		...
		adr.$('<div>').addClass('clear').appendTo(attackBtns);

		const healBtns = adr.$('<div>').appendTo(btns).attr('id', 'healButtons');
		...
		adr.$('<div>').addClass('clear').appendTo(healBtns);
		...
		Events._specialTimers = (scene.specials ?? []).map((s: Special) => Engine.setInterval(
			() => {
				const enemy = adr.$('#enemy');
				const text = s.action(enemy);
				Events.updateFighterDiv(enemy);
				if (text) {
					Events.drawFloatText(text, adr.$('.hp', enemy))
				}
			},
			s.delay * 1000
		));
	...
	static startEnemyAttacks = (delay?: number) => {
		adr.clearInterval(Events._enemyAttackTimer);
	...
	static drawFloatText(text: string, parent: Query, cb?: VoidFunction) {
		adr.$('<div>').text(text).addClass('damageText').appendTo(parent).animate({
			'bottom': '70px',
			'opacity': '0'
		},
			700,
		...
		if (Events.activeEvent()) {
			...
			if (weapon.cost) {
				...
				if (out) {
					...
					const weaponBtn = adr.$('.weaponButton');
			...
			attackFn(adr.$('#wanderer'), dmg, function () {
				const enemy = adr.$('#enemy');
				...
				if (enemyHp <= 0 && !Events.won) {
					...
					Events.won = true;
					if (explosion) {
						Events.explode(enemy, adr.$('#wanderer'), explosion);
					}
					else {
						Events.winFight();
					}
	...
	static dotDamage = (target: Query, dmg: number) => {
		...
		Events.drawFloatText(`-${dmg}`, adr.$('.hp', target));
	};
	...
	static damage(fighter: Query, enemy: Query, dmg: string | number, type: string, cb?: VoidFunction) {
		...
		if (typeof dmg === 'number') {
			if (dmg < 0) {
				...
				if (venomous && !shielded) {
					adr.clearInterval(Events._dotTimer);
					Events._dotTimer = adr.setInterval(() => {
						if (typeof dmg !== 'number') {
			...
			if (dmg === 'stun') {
				...
				adr.setTimeout(() => enemy.data('stunned', false), Events.STUN_DURATION);
			}
		}
		...
		Events.drawFloatText(msg.toString(), adr.$('.hp', enemy), cb);
	}
	...
	static animateRanged(fighter: Query, dmg: string | number, callback: VoidFunction) {
		...
		adr.$('<div>').css(start).addClass('bullet').text('o').appendTo(adr.$('#description'))
	...
	static enemyAttack() {
		...
		if (!stunned && !meditating) {
			...
			attackFn(adr.$('#enemy'), dmg, Events.checkPlayerDeath);
		}
	};

	static checkPlayerDeath = () => {
		if (adr.$('#wanderer').data('hp') <= 0) {
			Events.clearTimeouts();
			Events.endEvent();
			World.die();
			return true;
	...
	static clearTimeouts = () => {
		adr.clearInterval(Events._enemyAttackTimer);
		Events._specialTimers.forEach(adr.clearInterval);
		adr.clearInterval(Events._dotTimer);
	};
	...
	static winFight() {
		Engine.setTimeout(function () {
			...
			adr.$('#enemy').animate({ opacity: 0 }, 300, function () {
				Engine.setTimeout(function () {
					let leaveBtn;
					const scene = Events.activeEvent().scenes[Events.activeScene];
					const desc = adr.$('#description', Events.eventPanel());
					...
					desc.empty();
					btns.empty();
					adr.$('<div>').text(scene.deathMessage).appendTo(desc);

					const takeETbtn = Events.drawLoot(scene.loot);

					const exitBtns = adr.$('<div>').appendTo(btns).attr('id', 'exitButtons');
					if (scene.buttons) {
						// Draw the buttons
					...
					adr.$('<div>').addClass('clear').appendTo(exitBtns);
	...
	static drawDrop(btn: Query) {
		...
		if (weight > freeSpace) {
			...
			let dropMenu: Query;
			if (adr.$('#dropMenu').found) {
				dropMenu = adr.$('#dropMenu');
				adr.$('#dropMenu').empty();
			} else {
				dropMenu = adr.$('<div>').attr({ 'id': 'dropMenu', 'data-legend': _('drop:') });
				needsAppend = true;
			}
			for (const k in Path.outfit) {
				...
				if (itemWeight > 0) {
					...
					if (numToDrop > 0) {
						const dropRow = adr.$('<div>').attr('id', 'drop_' + k.replace(/ /g, '-'))
	...
	static drawLootRow(name: string, num: number) {
		...
		const lootRow = adr.$('<div>').attr('id', 'loot_' + id).data('item', name).addClass('lootRow');
		...
		adr.$('<span>').insertBefore(takeall.find('.cooldown'));
		adr.$('<div>').addClass('clear').appendTo(lootRow);
		return lootRow;
	};

	static drawLoot(lootList: LootList) {
		const desc = adr.$('#description', Events.eventPanel());
		const lootButtons = adr.$('<div>').attr({ 'id': 'lootButtons', 'data-legend': _('take:') });
		...
		if (lootButtons.children().length > 0) {
			const takeETrow = adr.$('<div>').addClass('takeETrow');
			...
			adr.$('<span>').insertBefore(takeET.find('.cooldown'));
			adr.$('<div>').addClass('clear').appendTo(takeETrow);
			takeETrow.appendTo(lootButtons);
			Events.setTakeAll(lootButtons);
		} else {
			const noLoot = adr.$('<div>').addClass('noLoot').text(_('nothing to take'));
	...
	static dropStuff(takeItemBtn: Query, e: Event) {
		...
		const lootButtons = adr.$('#lootButtons');
		Engine.log('dropping ' + num + ' ' + thing);

		const lootBtn = adr.$('#' + id, lootButtons);
		if (lootBtn.found > 0) {
			...
			lootRow.insertBefore(adr.$('.takeETrow', lootButtons));
		}
			...
			if (weight <= freeSpace) {
				...
				if (num === 0) {
					...
					btn.animate({ 'opacity': 0 }, 300, function (this: Query) {
						this.removeParent();
						if (adr.$('#lootButtons').found) {
							adr.$('#lootButtons').remove();
						}
					});
				}
	...
	static takeAll(btn: Query) {
		const target = adr.$('#' + btn.attr('id').substring(4));
	...
	static takeEverything(btn: Query) {
		adr.$('#lootButtons').children().filter(row => row.hasClass('lootRow')).forEach(lootBtn => {
	...
	static createFighterDiv(chara: string, hp: number, maxhp: number) {
		const fighter = adr.$('<div>')
			.addClass('fighter')
			.data('hp', hp)
			.data('maxHp', maxhp)
			.data('refname', chara);
		adr.$('<div>').addClass('label').text(_(chara)).appendTo(fighter);
		adr.$('<div>').addClass('hp').text(hp + '/' + maxhp).appendTo(fighter);
		return fighter;
	};

	static updateFighterDiv(fighter: Query) {
		adr.$('.hp', fighter).text(fighter.data('hp') + '/' + fighter.data('maxHp'));
	...
	static startStory(scene: ADREvent) {
		...
		for (const i in scene.text) {
			adr.$('<div>').text(scene.text[i]).appendTo(desc);
		}

		if (scene.textarea !== null && scene.textarea !== undefined) {
			const ta = adr.$('<textarea>').val(scene.textarea).appendTo(desc);
			if (scene.readonly) {
				ta.attr('readonly', true);
			}
			Engine.autoSelect(adr.$('#description').find('textarea'));
		}
		...
		// Draw the buttons
		const exitBtns = adr.$('<div>').attr('id', 'exitButtons').appendTo(adr.$('#buttons', Events.eventPanel()));
		leaveBtn = Events.drawButtons(scene);
		adr.$('<div>').addClass('clear').appendTo(exitBtns);
	...
	static drawButtons(scene: ADREvent) {
		const btns = adr.$('#exitButtons', Events.eventPanel());
		const btnsList = [];
	...
	static updateButtons() {
		...
		for (const bId in btns) {
			const b = btns[bId];
			const btnEl = adr.$('#' + bId, Events.eventPanel());
	...
	static blinkTitle = function () {
		const title = adr.title() as string;

		// every 3 seconds change title to '*** EVENT ***', then 1.5 seconds later, change it back to the original title.
		Events.BLINK_INTERVAL = adr.setInterval(function () {
			adr.title(_('*** EVENT ***'));
			Engine.setTimeout(function () { adr.title(title); }, 1500, true);
		}, 3000);
	};

	static stopTitleBlink() {
		adr.clearInterval(Events.BLINK_INTERVAL);
	...
	static startEvent(event: ADREvent, options?: EventOptions) {
		...
		event.eventPanel = adr.$('<div>').attr('id', 'event').addClass('eventPanel').css('opacity', '0');
		if (options && options.width) {
			Events.eventPanel().css('width', options.width);
		}
		adr.$('<div>').addClass('eventTitle').text(Events.activeEvent().title).appendTo(Events.eventPanel());
		adr.$('<div>').attr('id', 'description').appendTo(Events.eventPanel());
		adr.$('<div>').attr('id', 'buttons').appendTo(Events.eventPanel());
		Events.loadScene('start');
		adr.$('#wrapper').append(Events.eventPanel());
