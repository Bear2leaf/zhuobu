...
import { _LEAVE_COOLDOWN } from "./executioner.js";

/**
 * Events that only occur at specific times. Launched manually.
 **/
export const EventsSetpieces: Record<string, Setpieces> = {
	...
	"swamp": { /* Swamp */
		...
		scenes: {
			...
			'talk': {
				...
				onLoad: function() {
					StateManager.addPerk('gastronome');
	...
	"city": { /* City */
		...
		scenes: {
			...
			'end1': {
				...
				onLoad: function() {
					World.clearDungeon();
					StateManager.set('game.cityCleared', true);
				},
				loot: {
					bullets: {
						min: 5,
