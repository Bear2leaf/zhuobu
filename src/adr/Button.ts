...
export default class Button {
	...
	static create(options: ButtonOptions) {
		...
		el.append(adr.$("<div>").addClass('cooldown'));
		...
		if (options.cost) {
			...
			const costTooltip = adr.$('<div>').addClass('tooltip ' + ttPos);
			for (const k in options.cost) {
				adr.$("<div>").addClass('row_key').text(_(k)).appendTo(costTooltip);
				adr.$("<div>").addClass('row_val').text(options.cost[k]).appendTo(costTooltip);
			}
	...
	static clearCooldown(btn: Query, cooldownEnded?: boolean) {
		...
		if (btn.data('countdown')) {
			adr.clearInterval(btn.data('countdown'));
