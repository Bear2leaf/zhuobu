...
export default class Fabricator {
  ...
  static init() {
    ...
    Fabricator.panel = adr.$('<div>').attr('id', "fabricatorPanel")
      .addClass('location');
    if (Ship.panel) {
      Fabricator.panel.insertBefore(Ship.panel);
    }
    else {
      Fabricator.panel.appendTo(adr.$('#locationSlider'));
    }
  ...
  static setTitle() {
    if(Engine.activeModule === Fabricator) {
      adr.title(_("A Whirring Fabricator"));
    }
  }

  static updateBuildButtons() {
    let section = adr.$('#fabricateButtons');
    let needsAppend = false;
    if (section.found === 0) {
      section = adr.$('<div>').attr({ 'id': 'fabricateButtons', 'data-legend': _('fabricate:') }).css('opacity', 0);
      needsAppend = true;
    }
    ...
    for (const key in Fabricator.Craftables) {
      ...
      if (!value.button) {
        ...
        // refresh the tooltip
        const costTooltip = adr.$('.tooltip', value.button);
        costTooltip.empty();
        const cost = value.cost();
        for (const resource in cost) {
          const num = cost[resource];
          adr.$("<div>").addClass('row_key').text(_(resource)).appendTo(costTooltip);
          adr.$("<div>").addClass('row_val').text(num).appendTo(costTooltip);
        }
  ...
  static updateBlueprints(ignoreStores?: boolean) {
    ...
    let blueprints = adr.$('#blueprints');
    let needsAppend = false;
    if(blueprints.found === 0) {
      needsAppend = true;
      blueprints = adr.$('<div>').attr({'id': 'blueprints', 'data-legend': _('blueprints')});
    }
    ...
    for (const k in StateManager.get('character.blueprints')) {
      ...
      if(StateManager.get(`character.blueprints.${k}`) && r.found === 0) {
        r = adr.$('<div>').attr('id', id).addClass('blueprintRow').appendTo(blueprints);
