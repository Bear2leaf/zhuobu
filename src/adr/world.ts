...
export default class World {
  ...
  static init(options?: WorldOptions) {
    ...
    this.panel = adr.$('<div>').attr('id', "worldPanel").addClass('location').appendTo(adr.$('#outerSlider'));

    // Create the shrink wrapper
    const outer = adr.$('<div>').attr('id', 'worldOuter').appendTo(this.panel);

    // Create the bag panel
    adr.$('<div>').attr('id', 'bagspace-world').append(adr.$('<div>')).appendTo(outer);
    adr.$('<div>').attr('id', 'backpackTitle').appendTo(outer);
    adr.$('<div>').attr('id', 'backpackSpace').appendTo(outer);
    adr.$('<div>').attr('id', 'healthCounter').appendTo(outer);
  ...
  static updateSupplies() {
    const supplies = adr.$('#bagspace-world').find('div');
    ...
    for (const k in Path.outfit) {
      let item = adr.$('#supply_' + k.replace(' ', '-'), supplies);
      ...
      if (num > 0 && item.found === 0) {
        ...
        adr.$('#' + item.attr('id'), supplies).text(_(k) + ':' + num);
      } else {
        item.remove();
      }
    }
    ...
    adr.$('#backpackTitle').text(t);

    // Update bagspace
    adr.$('#backpackSpace').text(_('free {0}/{1}', Math.floor(Path.getCapacity() - total).toString(), Path.getCapacity().toString()));
  };
  ...
  static setHp(hp: number) {
    if (typeof hp === 'number' && !isNaN(hp)) {
      ...
      adr.$('#healthCounter').text(_('hp: {0}/{1}', World.health.toString(), World.getMaxHealth().toString()));
    }
  };

  static createItemDiv(name: string, num: number) {
    const div = adr.$('<div>').attr('id', 'supply_' + name.replace(' ', '-'))
      .addClass('supplyItem')
  ...
  static drawMap() {
    let map = adr.$('#map');
    if (map.found === 0) {
      map = adr.$('<div>').attr('id', 'map').appendTo(adr.$('#worldOuter'));
  ...
  static die() {
    if (!World.dead) {
      ...
      adr.$('#outerSlider').animate({ opacity: '0' }, 600, function () {
        adr.$('#outerSlider').css('left', '0px');
        adr.$('#locationSlider').css('left', '0px');
        adr.$('#storesContainer').css({ 'top': '0px', 'right': '0px' });
        Engine.activeModule = Room;
        adr.$('#header').children().forEach(v => v.removeClass('selected'));
        Room.tab.addClass('selected');
        Engine.setTimeout(function () {
          Room.onArrival();
          adr.$('#outerSlider').animate({ opacity: '1' }, 600);
          Button.cooldown(adr.$('#embarkButton'));
  ...
  static goHome() {
    ...
    adr.$('#outerSlider').animate({ left: '0px' }, 300);
  ...
  static onArrival() {
    ...
    Button.clearCooldown(adr.$('#embarkButton'));
    Engine.keyLock = false;
    ...
    World.dead = false;
    adr.$('#bagspace-world').find('div').empty();
    World.updateSupplies();
    adr.$('#bagspace-world').width(adr.$('#map').width());
  };

  static setTitle() {
    adr.title(_('A Barren World'));
  };
