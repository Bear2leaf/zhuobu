...
export default class HorizontalQuad {
    ...
    constructor(left: number, top: number, width: number, height: number, color: Vec4 = new Vec4(1, 1, 1, 1), initIndex: number = 0) {
        this.a = new Point(left, 0, top, undefined, color, 0);
        this.b = new Point(left, 0, top + height, undefined, color, 1);
        this.c = new Point(left + width, 0, top + height, undefined, color, 2);
        this.d = new Point(left + width, 0, top, undefined, color, 3);
        this.indices = [
    ...
    copyHeight(other: HorizontalQuad) {
