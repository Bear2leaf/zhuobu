...
export default class Room {
	...
	static init(options?: RoomOptions) {
		...
		Query.$('<div>').attr('id', 'storesContainer').prependTo(Query.$('#roomPanel'));
	...
	static setTitle() {
		...
		Query.$('#location_room').text(title);
	};

	static updateButton() {
		const light = Query.$('#lightButton');
		const stoke = Query.$('#stokeButton');
	...
	static lightFire() {
		...
		if (wood < 5) {
			...
			Button.clearCooldown(Query.$('#lightButton'));
			return;
		} else if (wood > 4) {
	...
	static stokeFire() {
		...
		if (wood === 0) {
			...
			Button.clearCooldown(Query.$('#stokeButton'));
			return;
		}
		if (wood > 0) {
	...
	static updateStoresView() {
		...
		if (resources.found === 0) {
			resources = Query.$('<div>').attr({
				id: 'resources'
			}).css('opacity', 0);
			rNeedsAppend = true;
		}
		if (special.found === 0) {
			special = Query.$('<div>').attr({
				id: 'special'
			}).css('opacity', 0);
			sNeedsAppend = true;
		}
		if (weapons.found === 0) {
			weapons = Query.$('<div>').attr({
				'id': 'weapons',
		...
		for (const k in StateManager.get('stores')) {
			...
			if (row.found === 0) {
				row = Query.$('<div>').attr('id', id).addClass('storeRow');
				Query.$('<div>').addClass('row_key').text(lk).appendTo(row);
				Query.$('<div>').addClass('row_val').text(Math.floor(num)).appendTo(row);
		...
		if (needsAppend && stores.find('.storeRow').found > 0) {
			stores.appendTo(Query.$('#storesContainer'));
		...
		if (wNeedsAppend && weapons.children().length > 0) {
			weapons.appendTo(Query.$('#storesContainer'));
	...
	static updateIncomeView() {
		...
		stores.children().forEach(function (storeRow, index) {
			...
			const tt = Query.$('<div>').addClass('tooltip ' + ttPos);
			...
			for (const incomeSource in StateManager.get('income')) {
				...
				for (const store in income.stores) {
					if (store === storeName && income.stores[store] !== 0) {
						Query.$('<div>').addClass('row_key').text(_(incomeSource)).appendTo(tt);
						Query.$('<div>')
							.addClass('row_val')
			...
			if (tt.children().length > 0) {
				...
				Query.$('<div>').addClass('total row_key').text(_('total')).appendTo(tt);
				const delay = totalIncome[storeName].delay;
				if (delay === undefined) {
					throw new Error("delay is undefined");
				}
				Query.$('<div>').addClass('total row_val').text(Engine.getIncomeMsg(total, delay.toString())).appendTo(tt);
	...
	static updateBuildButtons() {
		let buildSection = Query.$('#buildBtns');
		let needsAppend = false;
		if (buildSection.found === 0) {
			buildSection = Query.$('<div>').attr({ 'id': 'buildBtns', 'data-legend': _('build:') }).css('opacity', 0);
			needsAppend = true;
		}

		let craftSection = Query.$('#craftBtns');
		let cNeedsAppend = false;
		if (craftSection.found === 0 && StateManager.get('game.buildings.workshop', true) > 0) {
			craftSection = Query.$('<div>').attr({ 'id': 'craftBtns', 'data-legend': _('craft:') }).css('opacity', 0);
			cNeedsAppend = true;
		}

		let buySection = Query.$('#buyBtns');
		let bNeedsAppend = false;
		if (buySection.found === 0 && StateManager.get('game.buildings.trading post', true) > 0) {
			buySection = Query.$('<div>').attr({ 'id': 'buyBtns', 'data-legend': _('buy:') }).css('opacity', 0);
			bNeedsAppend = true;
		}

		for (const k in Room.Craftables) {
			const craftable = Query.craftable = Room.Craftables[k];
			...
			if (!craftable.button) {
				...
				// refresh the tooltip
				const costTooltip = Query.$('.tooltip', craftable.button);
				costTooltip.empty();
				const cost = craftable.cost();
				for (const c in cost) {
					Query.$("<div>").addClass('row_key').text(_(c)).appendTo(costTooltip);
					Query.$("<div>").addClass('row_val').text(cost[c]).appendTo(costTooltip);
				}
		...
		for (const g in Room.TradeGoods) {
			const good = Query.good = Room.TradeGoods[g];
			...
			if (!good.button) {
				...
				// refresh the tooltip
				const goodsCostTooltip = Query.$('.tooltip', good.button);
				goodsCostTooltip.empty();
				const goodCost = good.cost();
				for (const gc in goodCost) {
					Query.$("<div>").addClass('row_key').text(_(gc)).appendTo(goodsCostTooltip);
					Query.$("<div>").addClass('row_val').text(goodCost[gc]).appendTo(goodsCostTooltip);
				}
	...
	static compassTooltip(direction: string) {
		const ttPos = Query.$('#resources').children().length > 10 ? 'top right' : 'bottom right';
		const tt = Query.$('<div>').addClass('tooltip ' + ttPos);
		Query.$('<div>').addClass('row_key').text(_('the compass points ' + direction)).appendTo(tt);
		tt.appendTo(Query.$('#row_compass'));
	};
