...
export default class Space {
	...
	static init(options?: SpaceOptions) {
		...
		this.panel = Query.$('<div>').attr('id', "spacePanel")
			.addClass('location')
			.appendTo(Query.$('#outerSlider'));

		// Create the ship
		Space.ship = Query.$('<div>').text("@").attr('id', 'ship').appendTo(this.panel);

		// Create the hull display
		const h = Query.$('<div>').attr('id', 'hullRemaining').appendTo(this.panel);
		Query.$('<div>').addClass('row_key').text(_('hull: ')).appendTo(h);
		Query.$('<div>').addClass('row_val').appendTo(h);
	...
	static onArrival() {
		...
		Space.startAscent();
		Space._shipTimer = Query.setInterval(Space.moveShip, 33);
		Space._volumeTimer = Query.setInterval(Space.lowerVolume, 1000);
		AudioEngine.playBackgroundMusic(AudioLibrary.MUSIC_SPACE);
	};
	...
	static updateHull() {
		Query.$('#hullRemaining .row_val', Space.panel).text(Space.hull + '/' + Ship.getMaxHull());
	};
	...
	static createAsteroid(noNext?: boolean) {
		...
		const a = Query.$('<div>').addClass('asteroid').text(c).appendTo(Query.$('#spacePanel').css('left', x + 'px'));
		a.data("xMin", x);
	...
	static startAscent() {
		...
		Query.$('body').addClass('noMask').css({ backgroundColor: body_color }).animate({
			backgroundColor: to_color
		},Space.FTB_SPEED, Space.endGame);
		Space.drawStars();
		Space._timer = Query.setInterval(function () {
			Space.altitude += 1;
			if (Space.altitude % 10 === 0) {
				Space.setTitle();
			}
			if (Space.altitude > 60) {
				Query.clearInterval(Space._timer);
			}
		}, 1000);

		Space._panelTimeout = Engine.setTimeout(function () {
			if (Engine.isLightsOff())
				Query.$('#spacePanel, .menu, select.menuBtn').animate({ color: '#272823' }, 500);
			else
				Query.$('#spacePanel, .menu, select.menuBtn').animate({ color: 'white' }, 500);
	...
	static drawStars(duration?: number) {
		const starsContainer = Query.$('<div>').attr('id', 'starsContainer').appendTo(Query.$('body'));
		Space.stars = Query.$('<div>').css('bottom', '0px').attr('id', 'stars').appendTo(starsContainer);
		let s1 = Query.$('<div>').css({
			width: Space.STAR_WIDTH + 'px',
			height: Space.STAR_HEIGHT + 'px'
		});
		let s2 = Query.$('<div>').css({
			width: Space.STAR_WIDTH + 'px',
			height: Space.STAR_HEIGHT + 'px'
		});;
		...
		Space.starsBack = Query.$('<div>').css('bottom', '0px').attr('id', 'starsBack').appendTo(starsContainer);
		s1 = Query.$('<div>').css({
			width: Space.STAR_WIDTH + 'px',
			height: Space.STAR_HEIGHT + 'px'
		});
	...
	static drawStarAsync(el: Query, el2: Query, num: number) {
		...
		Query.$('<div>').text('.').addClass('star').css({
			top: top,
			left: left
		}).appendTo(el);
		Query.$('<div>').text('.').addClass('star').css({
	...
	static crash() {
		...
		Space.done = true;
		Query.clearInterval(Space._timer);
		Query.clearInterval(Space._shipTimer);
		Query.clearInterval(Space._volumeTimer);
		Query.clearTimeout(Space._panelTimeout);
		let body_color;
		if (Engine.isLightsOff())
			body_color = '#272823';
		else
			body_color = '#FFFFFF';
		// Craaaaash!
		Query.$('body').removeClass('noMask').animate({
			backgroundColor: body_color
		}, 300, function () {
			Space.stars?.remove();
			Space.starsBack?.remove();
			Space.stars = Space.starsBack = null;
			Query.$('#starsContainer').remove();
			Query.$('body').attr('style', '');
			Query.$('#notifyGradient').attr('style', '');
			Query.$('#spacePanel').attr('style', '');
		});
		Query.$('.menu, select.menuBtn').animate({ color: '#666' }, 300);
		Query.$('#outerSlider').animate({ top: '0px' }, 300);
		Engine.activeModule = Ship;
		Ship.onArrival();
		Button.cooldown(Query.$('#liftoffButton'));
	...
	static endGame() {
		...
		Query.clearTimeout(Outside._popTimeout);
		Query.clearTimeout(Engine._incomeTimeout);
		Query.clearTimeout(Events._eventTimeout);
		Query.clearTimeout(Room._fireTimer);
		Query.clearTimeout(Room._tempTimer);
		...
		Query.$('#hullRemaining', Space.panel).animate({ opacity: 0 }, 500);
		Space.ship.animate({
			top: '350px',
			left: '240px'
		}, 3000, function () {
			Engine.setTimeout(function () {
				Space.ship.animate({
					...
					Query.$('#outerSlider').css({ 'left': '0px', 'top': '0px' });
					Query.$('#locationSlider, #worldPanel, #spacePanel, #notifications').remove();
					Query.$('#header').empty();
					Engine.setTimeout(function () {
						let container_color;
						if (Engine.isLightsOff())
						...
						Query.$('#starsContainer').animate({
							...
							function () {
								...
								Prestige.save();
								Query.$('#starsContainer').remove();
								Query.$('#content, #notifications').remove();
	...
	static showEndingOptions() {
		...
		Query.$('<span>')
			.addClass('endGame endGameOption')
			.text(_('iOS.'))
			.click(function () { open('https://itunes.apple.com/app/apple-store/id736683061?pt=2073437&ct=gameover&mt=8'); })
			.appendTo(Query.$('.centerCont'))
			.animate({ opacity: 1 }, 1500);
		Query.$('<br />')
			.appendTo(Query.$('.centerCont'));
		Query.$('<span>')
			.addClass('endGame endGameOption')
			.text(_('android.'))
			.click(function () { open('https://play.google.com/store/apps/details?id=com.yourcompany.adarkroom'); })
			.appendTo(Query.$('.centerCont'))
			.animate({ opacity: 1 }, 1500);
	};
