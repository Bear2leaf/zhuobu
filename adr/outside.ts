...
export default class Outside {
	...
	static init(options?: OutsideOptions) {
		...
		this.panel = Query.$('<div>').attr('id', "outsidePanel")
			.addClass('location')
	...
	static updateWorkersView() {
		...
		if (workers.found === 0) {
			needsAppend = true;
			workers = Query.$('<div>').attr('id', 'workers').css('opacity', 0);
		}

		let numGatherers = StateManager.get('game.population');
		let gatherer = Query.$('#workers_row_gatherer', workers);

		for (const k in StateManager.get('game.workers')) {
			const lk = _(k);
			const workerCount = StateManager.get('game.workers.' + k + '');
			let row = Query.$('#workers_row_' + k.replace(' ', '-'), workers);
			if (row.found === 0) {
		...
		if (numGatherers === 0) {
			Query.$('.upBtn', workers).addClass('disabled');
			Query.$('.upManyBtn', workers).addClass('disabled');
		} else {
			Query.$('.upBtn', workers).removeClass('disabled');
			Query.$('.upManyBtn', workers).removeClass('disabled');
		}


		if (needsAppend && workers.children().length > 0) {
			workers.appendTo(Query.$('#outsidePanel')).animate({ opacity: 1 }, 300);
		}
	};
	...
	static makeWorkerRow(key: string, num: number) {
		...
		const row = Query.$('<div>')
			...
			.addClass('workerRow');
		Query.$('<div>').addClass('row_key').text(name).appendTo(row);
		const val = Query.$('<div>').addClass('row_val').appendTo(row);

		Query.$('<span>').text(num).appendTo(val);

		if (key !== 'gatherer') {
			Query.$('<div>').addClass('upBtn').appendTo(val).click(function () { Outside.increaseWorker(row, 1) });
			Query.$('<div>').addClass('dnBtn').appendTo(val).click(function () { Outside.decreaseWorker(row, 1) });
			Query.$('<div>').addClass('upManyBtn').appendTo(val).click(function () { Outside.increaseWorker(row, 1) });
			Query.$('<div>').addClass('dnManyBtn').appendTo(val).click(function () { Outside.decreaseWorker(row, 1) });
		}

		Query.$('<div>').addClass('clear').appendTo(row);

		const tooltip = Query.$('<div>').addClass('tooltip bottom right').appendTo(row);
		const income = Outside._INCOME[key];
		for (const s in income.stores) {
			const r = Query.$('<div>').addClass('storeRow');
			Query.$('<div>').addClass('row_key').text(_(s)).appendTo(r);
			if (income.delay === undefined) {
				throw new Error('income.delay is undefined');
			}
			Query.$('<div>').addClass('row_val').text(Engine.getIncomeMsg(income.stores[s], income.delay.toString())).appendTo(r);
			r.appendTo(tooltip);
		}
	...
	static updateVillageRow(name: string, num: number, village: Query) {
		...
		const lname = _(name);
		let row = Query.$('#' + id, village);
		if (row.found === 0 && num > 0) {
			row = Query.$('<div>').attr('id', id).addClass('storeRow');
			Query.$('<div>').addClass('row_key').text(lname).appendTo(row);
			Query.$('<div>').addClass('row_val').text(num).appendTo(row);
			Query.$('<div>').addClass('clear').appendTo(row);
			let curPrev = null;
	...
	static updateVillage(ignoreStores?: boolean) {
		let village = Query.$('#village');
		let population = Query.$('#population');
		let needsAppend = false;
		if (village.found === 0) {
			needsAppend = true;
			village = Query.$('<div>').attr('id', 'village').css('opacity', 0);
			population = Query.$('<div>').attr('id', 'population').appendTo(village);
		}
		...
		if (needsAppend && village.children().length > 1) {
			village.prependTo(Query.$('#outsidePanel'));
		...
		if (!ignoreStores && Engine.activeModule === Outside && village.children().length > 1) {
			Query.$('#storesContainer').css({ top: village.height() + 26 + Outside._STORES_OFFSET + 'px' });
		}
	};
	...
	static updateVillageIncome() {
		for (const worker in Outside._INCOME) {
			...
			if (typeof num === 'number' && num !== 0) {
				const stores: Stores = {};
				if (num < 0) num = 0;
				const tooltip = Query.$('#workers_row_' + worker.replace(' ', '-')).find('.tooltip');
				tooltip.empty();
				...
				for (const store in income.stores) {
					...
					const row = Query.$('<div>').addClass('storeRow');
					Query.$('<div>').addClass('row_key').text(_(store)).appendTo(row);
					if (income.delay === undefined) {
						throw new Error('income.delay is undefined');
					}
					Query.$('<div>').addClass('row_val').text(Engine.getIncomeMsg(stores[store], income.delay.toString())).appendTo(row);
	...
	static setTitle() {
		...
		Query.$('#location_outside').text(title);
	};
	...
	static onArrival(transition_diff?: number) {
		...
		Engine.moveStoresView(Query.$('#village'), transition_diff);
