...
export const EventsOutside = [
	...
	title: _('A Ruined Trap'),
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.buildings.trap', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				onLoad: function() {
					const numWrecked = Math.floor(Math.random() * StateManager.get('game.buildings.trap', true)) + 1;
					StateManager.add('game.buildings.trap', -numWrecked);
	...
	{ /* Hut fire */
		title: _('Fire'),
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.buildings.hut', true) > 0 && StateManager.get('game.population', true) > 50;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Sickness */
		title: _('Sickness'),
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.population', true) > 10 && StateManager.get('game.population', true) < 50 && StateManager.get('stores.medicine', true) > 0;
		},
		scenes: {
			'start': {
				text: [
			...
			'death': {
				...
				onLoad: function() {
					const numKilled = Math.floor(Math.random() * Math.floor(StateManager.get('game.population', true)/2)) + 1;
	...
	{ /* Plague */
		title: _('Plague'),
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.population', true) > 50 && StateManager.get('stores.medicine', true) > 0;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Beast attack */
		...
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.population', true) > 0;
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Soldier attack */
		...
		isAvailable: function() {
			return Engine.activeModule === Outside && StateManager.get('game.population', true) > 0 && StateManager.get('game.cityCleared');
		},
		scenes: {
			'start': {
				text: [
