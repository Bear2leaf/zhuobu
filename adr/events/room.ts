...
export const EventsRoom = [
	{ /* The Nomad  --  Merchant */
		...
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('stores.fur', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				buttons: {
					...
					'buyCompass': {
						available: function () {
							return StateManager.get('stores.compass', true) < 1;
						},
						text: _('buy compass'),
	...
	{ /* Noises Outside  --  gain wood/fur */
		title: _('Noises'),
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('stores.wood');
		},
		scenes: {
			'start': {
				text: [
	...
	{ /* Noises Inside  --  trade wood for better good */
		...
		scenes: {
			...
			scales: {
				...
				onLoad: function () {
					let numWood = StateManager.get('stores.wood', true);
					numWood = Math.floor(numWood * 0.1);
					if (numWood === 0) numWood = 1;
					let numScales = Math.floor(numWood / 5);
					if (numScales === 0) numScales = 1;
					StateManager.addM('stores', { 'wood': -numWood, 'scales': numScales });
				},
				buttons: {
					'leave': {
						text: _('leave'),
			...
			teeth: {
				...
				onLoad: function () {
					...
					StateManager.addM('stores', { 'wood': -numWood, 'teeth': numTeeth });
				},
				buttons: {
					'leave': {
						text: _('leave'),
			...
			cloth: {
				...
				onLoad: function () {
					...
					StateManager.addM('stores', { 'wood': -numWood, 'cloth': numCloth });
				},
				buttons: {
					'leave': {
						text: _('leave'),
	...
	{ /* The Beggar  --  trade fur for better good */
		title: _('The Beggar'),
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('stores.fur');
		},
		scenes: {
			start: {
				text: [
	...
	{/* The Shady Builder */
		...
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('game.buildings.hut', true) >= 5 && StateManager.get('game.buildings.hut', true) < 20;
		},
		scenes: {
			'start': {
				text: [
			...
			'build': {
				...
				onLoad: function () {
					...
					if (n < 20) {
						StateManager.set('game.buildings.hut', n + 1);
					}
				},
				buttons: {
					'end': {
	...
	{ /* Mysterious Wanderer  --  wood */
		...
		scenes: {
			...
			'wood100': {
				...
				action: function (inputDelay: number) {
					const delay = inputDelay;
					Events.saveDelay(function () {
						StateManager.add('stores.wood', 300);
			...
			'wood500': {
				...
				action: function (inputDelay: number) {
					const delay = inputDelay;
					Events.saveDelay(function () {
						StateManager.add('stores.wood', 1500);
						Notifications.notify(Room, _('the mysterious wanderer returns, cart piled high with wood.'));
		...
		scenes: {
			...
			'fur100': {
				...
				action: function (inputDelay: number) {
					const delay = inputDelay;
					Events.saveDelay(function () {
						StateManager.add('stores.fur', 300);
			...
			'fur500': {
				...
				action: function (inputDelay: number) {
					const delay = inputDelay;
					Events.saveDelay(function () {
						StateManager.add('stores.fur', 1500);
						Notifications.notify(Room, _('the mysterious wanderer returns, cart piled high with furs.'));
	...
	{ /* The Scout  --  Map Merchant */
		title: _('The Scout'),
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('features.location.world');
		},
		scenes: {
			'start': {
				text: [
				...
				buttons: {
					...
					'learn': {
						...
						available: function () {
							return !StateManager.hasPerk('scout');
						},
						onChoose: function () {
							StateManager.addPerk('scout');
						}
					},
					'leave': {
						text: _('say goodbye'),
			...
			'agree': {
				...
				buttons: {
					'evasion': {
						text: _('evasion'),
						available: function () {
							return !StateManager.hasPerk('evasive');
						},
						onChoose: function () {
							StateManager.addPerk('evasive');
						},
						nextScene: 'end'
					},
					'precision': {
						text: _('precision'),
						available: function () {
							return !StateManager.hasPerk('precise');
						},
						onChoose: function () {
							StateManager.addPerk('precise');
						},
						nextScene: 'end'
					},
					'force': {
						text: _('force'),
						available: function () {
							return !StateManager.hasPerk('barbarian');
						},
						onChoose: function () {
							StateManager.addPerk('barbarian');
						},
						nextScene: 'end'
					},
					'nothing': {
	...
	{ /* The Sick Man */
		...
		isAvailable: function () {
			return Engine.activeModule === Room && StateManager.get('stores.medicine', true) > 0;
		},
		scenes: {
			'start': {
				text: [
			...
			'cells': {
				...
				onLoad: function () {
					StateManager.add('stores.energy cell', 3);
				},
				buttons: {
					'bye': {
						text: _('say goodbye'),
			...
			'scales': {
				...
				onLoad: function () {
					StateManager.add('stores.scales', 5);
				},
				buttons: {
					'bye': {
						text: _('say goodbye'),
